...
router.get('/hdwallet_payment', (req, res, next) => {
    const config = req.app.config;
    let paymentType = '';
    if(req.session.cartSubscription){
        paymentType = '_subscription';
    ...
    // show bitcoin address and wait for payment, subscribing to wss
    res.render(`${config.themeViews}checkout-hdwallet`, {
        title: 'Checkout HD-WALLET- Payment',
        config: req.app.config,
        paymentConfig: getPaymentConfig(),
        session: req.session,
        paymentPage: true,
...
router.get('/hdwallet_payment_success', (req, res, next) => {
  
    const config = req.app.config;
    let paymentType = '';
    res.render(`${config.themeViews}payment-complete-hdwallet`, {
